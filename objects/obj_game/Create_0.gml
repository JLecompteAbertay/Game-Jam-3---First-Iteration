...
audio_play_sound(snd_music_bg, 0, true);
...
// Create the player in the center of the room.
instance_create_layer(room_width / 2, room_height / 2, "Instances", obj_player); //this works
...
spawn_enemy = function()
{
	...
	// By default this will be the Four of Hearts enemy.
	var _enemy = obj_four_heart;

	// If we are over level 2...
	if (global.level > 2)
	{
		// Change the enemy type to either
		// Ace of Spades or Four of Hearts.
		_enemy = choose(obj_ace_spade, obj_four_heart);
	}
	...
	{
		// Change the enemy type to either
		// Ace of Spades, Four of Hearts, or King of Clubs
		_enemy = choose(obj_ace_spade, obj_four_heart, obj_king_club);
	}
	...
	var _x = obj_player.x + lengthdir_x(20, _dir);

	// Then we get the position 1200 pixels away
	// from the hero on the y axis.
	var _y = obj_player.y + lengthdir_y(20, _dir);
		...
		camera_get_view_y(view_camera[view_current]) - 100 < _y &&  _y < camera_get_view_y(view_camera[view_current]) + camera_get_view_height(view_camera[view_current]) + 100)
	{
		...
		_x = obj_player.x + lengthdir_x(1200 + _i * 400, _dir);
		_y = obj_player.y + lengthdir_y(1200 + _i * 400, _dir);
